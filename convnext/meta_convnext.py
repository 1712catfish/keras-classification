...
from keras_cv_attention_models.convnext.convnext import add_with_layer_scale_and_drop_block
from keras_cv_attention_models.download_and_load import reload_model_weights
from keras_cv_attention_models.convnext import (
...
PRETRAINED_DICT = {
    ...
    "convnext_xlarge": {
        "imagenet21k-ft1k": {224: "7c7ab46f41ac34655f3e035b873a2163", 384: "636db850c0a73ba10e8ab32e91c38df6"}},
...
def convnext_block(inputs, output_channel, layer_scale_init_value=1e-6, use_grn=False, drop_rate=0, activation="gelu",
                   name=""):
    ...
    nn = nn if backend.image_data_format() == "channels_last" else layers.Permute((2, 3, 1), name=name + "permute_pre")(
        nn)
    ...
    nn = nn if backend.image_data_format() == "channels_last" else layers.Permute((3, 1, 2),
                                                                                  name=name + "permute_post")(nn)

    return add_with_layer_scale_and_drop_block(inputs, nn, layer_scale=layer_scale_init_value, drop_rate=drop_rate,
                                               name=name)
...
def MetaConvNeXt(
    ...
    if num_classes > 0:
        ...
        nn = layers.Dense(
            num_classes, dtype="float32", activation=classifier_activation, kernel_initializer=head_init,
            bias_initializer=head_init, name="predictions"
        )(nn)
